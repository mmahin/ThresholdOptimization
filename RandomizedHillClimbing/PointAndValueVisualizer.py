...
def PointAndValueVisualizer(point_x,point_y,Values, threshold1_set,threshold2_set, X_label,Y_label,Z_Label):
    xtics = []
    ytics = []
    for item in threshold1_set:
        xtics.append(str(round(item,2)))
    for item in threshold2_set:
        ytics.append(str(round(item,2)))
    fig, ax = plt.subplots()

    cmap = plt.get_cmap('viridis', 5)
    sc = ax.scatter(point_x, point_y, c= Values, cmap='viridis', vmin=0, vmax=1, alpha=1)
    ...
    #cbar = plt.colorbar(mappable = scatter, ticks = [0,0.1,0.2,0.3,0.,300,350,400])
    #cbar.ax.set_yticklabels(['0.05','0.10','0.15','0.20','0.25','0.30','0.35','0.40'],fontsize=18)
    cbar.set_label(label=Z_Label, size=11)
    ax.set_xlabel(X_label,size=11)
    ax.set_ylabel(Y_label,size=11)
...
target_threshold1_start = min(variable1_df['values'])
target_threshold1_end = max(variable2_df['values'])
target_threshold1_cutpoint = (target_threshold1_end - target_threshold1_start )/target_threshold1

target_threshold2_start = min(variable2_df['values'])
target_threshold2_end = max(variable2_df['values'])
