    ...
    for k in range(l,len(variable_names)):
        ...
        if variable1_name != variable2_name:# and table_name  in already_completed:
            ...
            for agreement in agreement_set:
                if agreement > 0.5 and expected_set[a_count] > 0.1:
                    patterns.append("{("+variable1_name+" "+ str(round(threshold1_set[a_count],2))+"),("+variable2_name+" "+str(round(threshold2_set[a_count],2))+")}")
                    imp_agreement.append(agreement)
                    imp_agreement_expected.append(expected_set[a_count])
                    threhsold1_imp_agreement.append(threshold1_set[a_count])
                    threhsold2_imp_agreement.append(threshold2_set[a_count])
                    if str(float(threshold1_set[a_count])) in threshold_area_dict[variable1_name].keys():
                        threhsold1_imp_agreement_area_cover.append(threshold_area_dict[variable1_name][str(float(threshold1_set[a_count]))])
                    else:
                        threhsold1_imp_agreement_area_cover.append(-1)
                    if str(float(threshold2_set[a_count])) in threshold_area_dict[variable2_name].keys():
                        threhsold2_imp_agreement_area_cover.append(threshold_area_dict[variable2_name][str(float(threshold2_set[a_count]))])
                    else:
                        threhsold2_imp_agreement_area_cover.append(-1)
                    imp_agreement_lift.append(lift_set[a_count])
                a_count+= 1
            '''
            c.execute(f'SELECT * FROM {variable1_name}')
            rows = c.fetchall()
