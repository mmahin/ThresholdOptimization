...
def AgreementValueGeneratorForTwoThresholdsWithAlternativeAgreement(min_threshold1, cutpoint_threshold1, min_threshold2,
                                                              ...
                                                              variable2_value_matrix,grid_row_size, grid_column_size,steps):
    agreements = []
    threshold1_values = []
    ...
    for i in range(0,steps):
        ...
        for j in range(0,steps):
            if str(threshold2) not in countAboveThreshold2dict:
                countAboveThreshold2dict[str(threshold2)] = countAboveThreshold(variable2_value_matrix, threshold2)
            count_above_threshold2 = countAboveThreshold2dict[str(threshold2)]
            ...
            if (threshold1>=0.35 and threshold1<=0.45) and (threshold2>=50 and threshold2<=60):
                print(i,j, threshold1, threshold2, countAboveThreshold1, count_above_threshold2,
                  countMutualPointsAboveThreshold1Threshold2, agreement)
                Is.append(i)
                Js.append(j)
    ...
    print(max(Is),min(Is),max(Js),min(Js))
